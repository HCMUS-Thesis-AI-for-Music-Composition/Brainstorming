...
for num in tqdm(os.listdir(root), desc="Extract attributes"):
    ...
    for idx, instrument in enumerate(I1s2):
        infer_command["gold_labels"][instrument] = infer_command["gold_labels"]["I1s2"][
            idx
        ]
    ...
    for midiname in os.listdir(midipath):
        ...
        extractor = DataExtractor(
            encoding_method="REMIGEN2", attribute_list_version="v3"
        )
        try:
        ...
        allinfermidivector[num][midiname] = {
            "value_dict": att_dict,
            "vector": {},
            "acc": {},
        }
    json.dump(
        prompt_value_dict, open(os.path.join(args.output, num, "value_dict.json"), "w")
    )
...
for num, info in tqdm(allmidiinfo.items(), desc="Comput Accuracy"):
    for midiname in allinfermidivector[num].keys():
        if (
            allinfermidivector[num][midiname]["value_dict"] == None
            or len(allinfermidivector[num][midiname]["value_dict"]) == 0
        ):
            ...
            gen_unit_dict = convert_value_dict_into_unit_dict(
                allinfermidivector[num][midiname]["value_dict"], midi_encoder
            )
            gen_vector = {}
            ...
            for idx, instrument in enumerate(I1s2):
                allinfermidivector[num][midiname]["vector"][instrument] = (
                    allinfermidivector[num][midiname]["vector"]["I1s2"][idx]
                )
            ...
            for att, vector in allinfermidivector[num][midiname]["vector"].items():
                ...
                #         cnt += 1
                #     allcnt += 1
                # print(f"vector ne {vector}")
                # print(f"nay :{info['gold_labels'][att]}")
                ...
                if att == "K1":
                    print("-------------------------")
                    print(f"vector ne : {vector}")
                    print(info["gold_labels"]["K1"])
                    print(vector == info["gold_labels"]["K1"])
                    if vector == info["gold_labels"]["K1"]:
                        cnt += 1
                        print("hit")
                    allcnt += 1
