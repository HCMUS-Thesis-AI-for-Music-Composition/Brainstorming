...
def generate_midi_obj_from_remigen_token_list(
    ...
    midi_obj.instruments = [
        miditoolkit.Instrument(program=(0 if i == 128 else i), is_drum=(i == 128), name=str(i))
    ...
    for item in token_list:  # Todo: 对错误的结果顺序做处理
        try:
            ...
            print(f"{item} is not a valid token.")
            raise
        if item_type == const.BAR_ABBR:
            ...
            if cur_ts_id != item_value:
                ...
                midi_obj.time_signature_changes.append(
                    miditoolkit.TimeSignature(numerator=cur_ts[0], denominator=cur_ts[1],
            ...
            if cur_tempo_id != item_value:
                ...
                midi_obj.tempo_changes.append(
                    miditoolkit.TempoChange(cur_tempo,
            ...
            if ignore_velocity:
                ...
                cur_inst.notes.append(
                    miditoolkit.Note(start=start_time, end=end_time, pitch=cur_pitch, velocity=cur_velocity)
                )
