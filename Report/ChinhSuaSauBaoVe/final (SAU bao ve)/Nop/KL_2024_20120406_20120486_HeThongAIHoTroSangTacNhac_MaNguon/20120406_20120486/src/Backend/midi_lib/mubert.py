...
class MuEmbeddings(BertEmbeddings):
    ...
    def forward(
        ...
        cls_input_ids = torch.tensor([cls_input_ids for _ in range(input_shape[0])]).to(
            token_type_ids.device
        )
        cls_token_type_ids = (
            torch.zeros((input_shape[0], len(self.num_labels)))
            .long()
...
class MusicBertAttention(BertAttention):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.self = MusicBertSelfAttention(
...
class MusicBertLayer(BertLayer):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = MusicBertAttention(
...
class MusicBertModel(BertModel):
    ...
    def forward(
        ...
        cls_attention_mask = (
            torch.tensor(
                [
                    [1 for _ in range(len(self.num_labels))]
        ...
        pooled_output = (
            self.pooler(sequence_output, 0) if self.pooler is not None else None
        )
...
class BertForAttributModel(BertPreTrainedModel):
    def __init__(self, config, num_labels, tokenizer):
        ...
        self.classifieratt = nn.ModuleList(
            [nn.Linear(config.hidden_size, v) for v in self.num_labels.values()]
        )
    ...
    def forward(
        ...
        if labels is not None:
            for k in labels.keys():
                ...
                loss[k] = loss_fct(
                    logits[k].view(-1, self.num_labels[k]), labels[k].view(-1)
                )
                total_loss += loss[k]
