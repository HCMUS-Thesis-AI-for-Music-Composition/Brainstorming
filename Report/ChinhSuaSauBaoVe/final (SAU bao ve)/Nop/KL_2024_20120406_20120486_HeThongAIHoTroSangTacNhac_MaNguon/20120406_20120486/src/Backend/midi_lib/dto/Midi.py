...
class MidiDTO:
    ...
    def __init__(self, **kwargs):
        ...
        self.key_signature_changes: list[KeySignatureChangeDTO] = list[
            KeySignatureChangeDTO
        ]([])
        self.time_signature_changes: list[TimeSignatureChangeDTO] = list[
        ...
        for key, value in kwargs.items():
            ...
            if key in type_of_keys:
                if not type(value) == type_of_keys[key]:
                    raise ValueError(
                        f"Expected {type_of_keys[key]} for {key}, got {type(value)}"
                    )
                else:
