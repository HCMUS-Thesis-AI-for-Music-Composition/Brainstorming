...
def attributes(args):
    ...
    # Load ensemble
    print(args.path.split(os.pathsep))
    ...
    for model in models:
        ...
        if not hasattr(args, 'generation'):
            args.generation = argparse.Namespace()
        args.generation.no_beamable_mm = False
    ...
    for j in range(args.need_num):
        for i in range(args.start, args.end):
            ...
            gen_command_list.append(
                [test_command[i]["infer_command_tokens"], f"{i}", j, test_command[i]]
            )
