...
sys.path.append("D:\\HCMUS\\Thesis\\Backend\\midi_lib\\")
sys.path.append("../../")
print(sys.path)
...
def midi_dto_to_midi_file_object_converter(midi_dto: MidiDTO) -> MidiFile:
    ...
    for tempo_change in midi_dto.tempo_changes:
        midi_file_object.tempo_changes.append(
            TempoChange(time=tempo_change.time, tempo=tempo_change.tempo)
        )
    ...
    for key_signature_change in midi_dto.key_signature_changes:
        ...
        if mc.ScaleName.MINOR.lower() in scale:
            ...
            if (
                f"{root_note}{scale}"
                not in mc.miditoolkit_supported_keys[mc.ScaleName.MINOR]
            ):
            ...
            if (
                f"{root_note}{scale}"
                not in mc.miditoolkit_supported_keys[mc.ScaleName.MAJOR]
            ):
                root_note = mu.equivalent_note_name(root_note)
            else:
                pass
        ...
        midi_file_object.key_signature_changes.append(
            KeySignature(time=key_signature_change.time, key_name=key_name)
        )
    ...
    for instrument_info in midi_dto.instruments:
        instrument = Instrument(
            name=instrument_info.name, program=instrument_info.program, notes=[]
        )
    ...
    for marker_info in midi_dto.markers:
        midi_file_object.markers.append(
            Marker(time=marker_info.time, text=marker_info.text)
        )
